...
func Example() {
	...
	// Try to obtain lock.
	lock, err := locker.Obtain(ctx, []string{"my-key", "my-key2"}, 100*time.Millisecond, nil)
...
func ExampleClient_Obtain_retry() {
	...
	lock, err := locker.Obtain(ctx, []string{"my-key", "my-key2"}, time.Second, &redislock.Options{
